...
public sealed class DryRunSuppressSaveChangesDbCommand : DryRunSuppressSaveChangesDbCommandTestBase
{
    [Fact]
    public void DryRunSuppressest()
    {
        ...
        Assert.True(TraceFlags.Match(true, [DryRunEventTrace.TransactionStarted, DryRunEventTrace.TransactionCommitting, DryRunEventTrace.TransactionCommitted]));
    ...
    public void DryRunSuppressNoTransactionTest()
    {
        ...
        DbContext.SaveChanges();
        DbContext.Database.ExecuteSqlRaw("INSERT INTO Entity1 (Id, Name) VALUES (2, 'Test2')");
        Assert.True(TraceFlags.Match(true, [DryRunEventTrace.CommandSuppressed, DryRunEventTrace.SaveChangesSuppressed]));
        Assert.False(DbContext.Set<Entity1>().Any());
        Assert.Single(DbContext.ChangeTracker.Entries());
        Assert.True(DbContext.ChangeTracker.Entries().All(e => e.State == EntityState.Added));
    }
